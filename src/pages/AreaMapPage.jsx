...
function getSceneMapPoint(scene) { return scene?.mapPoint || scene?.minimap || null; }
function getAlphabeticalFirstScene(area) {
  const scenes = Object.values(area?.tour?.scenes || {}).filter(Boolean);
  return scenes.sort((a, b) => getSceneTitle(a, a.id).localeCompare(getSceneTitle(b, b.id), undefined, { numeric: true, sensitivity: "base" }))[0] || null;
}
function getClosestScene(area, clickedPoint) {
  const scenes = Object.values(area?.tour?.scenes || {}).filter(Boolean);
  if (!clickedPoint || !scenes.length) return getAlphabeticalFirstScene(area);
  return scenes.map((scene) => ({ scene, point: getSceneMapPoint(scene) }))
    .filter((item) => item.point)
    .map((item) => ({ ...item, distance: Math.hypot(Number(item.point.x || 50) - clickedPoint.x, Number(item.point.y || 50) - clickedPoint.y) }))
    .sort((a, b) => a.distance - b.distance)[0]?.scene || getAlphabeticalFirstScene(area);
}
...
function getClosestArea(areas = [], clickedPoint) {
  ...
  return areas.filter((area) => area?.points).map((area) => {
    const center = getAreaCenter(area);
    return { area, distance: Math.hypot(center.x - clickedPoint.x, center.y - clickedPoint.y) };
...
function AreaMapPage() {
  ...
  useEffect(() => {
    ...
    getEffectiveFactoryMapsAsync({ force: true }).then((maps) => {
      if (!cancelled) { setFactoryMaps(maps); setLoading(false); }
    });
    return () => { cancelled = true; };
  }, []);

  const site = factoryMaps?.[siteId];
  const selectedAreaName = useMemo(() => selectedArea ? `Opening ${selectedArea.name}` : hoveredArea ? hoveredArea.name : "Click anywhere on the map", [hoveredArea, selectedArea]);
  ...
  function openArea(area, clickedPoint = null) {
    ...
    setTimeout(() => navigate(`/viewer/${site.id}/${area.id}${sceneQuery}`), 250);
  }
  function handleMapClick(event) {
    const clickedPoint = getClickPercent(event);
    openArea(getClosestArea(site.areas || [], clickedPoint), clickedPoint);
  }
    ...
    <div className={`area-page simple-map-page ${selectedArea ? "area-switching" : ""}`}>
      <main className="simple-map-body">
        <section className="area-map-card simple-map-card" onClick={handleMapClick} title="Click anywhere to open the nearest mapped area">
          {site.mapImage ? <img src={site.mapImage} alt={site.name} className="area-map-image" draggable="false" /> : <div className="area-map-image area-map-missing">No map image</div>}
          <svg className="area-map-svg" viewBox="0 0 100 100" preserveAspectRatio="none">
            {(site.areas || []).map((area) => (
              <polygon key={area.id} points={area.points} className={`clickable-area ${hoveredArea?.id === area.id ? "is-hovered" : ""} ${selectedArea?.id === area.id ? "is-selected" : ""}`}
                onMouseEnter={() => setHoveredArea(area)} onMouseLeave={() => setHoveredArea(null)}
                onClick={(event) => { event.stopPropagation(); openArea(area, getClickPercent(event)); }} />
            ))}
          </svg>
          <div className="map-floating-info">
            ...
            <div><div className="map-floating-kicker">SELECT AREA</div><div className="map-floating-title">{site.name}</div><div className="map-floating-subtitle">{selectedAreaName}</div></div>
          </div>
        </section>
      </main>
    </div>
