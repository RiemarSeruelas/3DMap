...
function StreetViewPage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      const maps = await getEffectiveFactoryMaps();
      if (!alive) return;
      setFactoryMaps(maps);
      setLoading(false);
    }
  ...
  if (loading) {
    return <div className="viewer-error-page">Loading 360 viewer...</div>;
  }
    ...
    return (
      <div className="viewer-error-page">
        <div className="viewer-error-card">
          <h1>No tour found</h1>
          <p>This mapped area does not have configured 360 locations yet.</p>
  ...
  return (
    <div className="viewer-page clean-viewer-page">
      <button className="floating-back-btn map-only-back-btn" onClick={() => navigate(`/map/${siteId}`)}>
        ← Map
      </button>

      <main className="clean-viewer-body">
        <StreetViewer mapData={area.tour} site={site} area={area} />
      </main>
    </div>
  );
}
